...
local timerNextFrostBomb        = mod:NewNextTimer(35, 64623)
...
function mod:Flames()	-- Flames 
	...
	mod:CountdownFinalSeconds(self.Options.SoundWarnCountingFlames, 28)
end
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif (msg == L.YellHardPull or msg:find(L.YellHardPull)) then -- register HARDMODE
		...
		timerFlameSuppressant:Start(75)	-- from vod, if blast first its delayed by ~4-5sec
		timerNextFlames:Start(6)
		self:ScheduleMethod(6, "Flames")
		if self.Options.WarnFlamesIn5Sec then
			warnFlamesIn5Sec:Schedule(1) 
		end
		mod:CountdownFinalSeconds(self.Options.SoundWarnCountingFlames, 6)
